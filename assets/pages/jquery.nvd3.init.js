...
(function ($) {
    ...
    nv.addGraph(function () {
        ...
        $.getJSON('http://localhost/poolworks.io/Hashratemonitor/getHasRates', function (data) {
            ...
            lineChart.xAxis.tickValues(d3.time.minute.range(
                    new Date(startDate),
                    new Date(endDate)),
                15); 
               
            lineChart.xAxis.tickFormat(function (d) {
                    return d3.time.format('%H-%i-%s')(new Date(d))
                });
            lineChart.yAxis.axisLabel('Voltage (v)').tickFormat(d3.format(',.2f'));
            d3.select('.line-chart svg').attr('perserveAspectRatio', 'xMinYMid').datum(dd).transition().duration(500).call(lineChart);
