...
class mainmodel extends CI_Model {
    ...
    public function deleteIframePosition($name, $d = 1) {
        ...
        $db = $this->db;
        $query = 'UPDATE `iframe_position_users` SET `status` = 0 WHERE `d` = ' . $d . ' AND `position_name` = ' . $this->db->escape($name) . ' AND user_id = ' . $user->id . ' LIMIT 1';
        $db->query($query);
    }
